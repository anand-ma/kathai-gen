...
def generate_story(image_url: str, mood: str, size, language: str):
    try:
        story_template = "Create a {size} {mood} story about the image attached in simple {language}."
        ...
        st.markdown("Story Prompt: ", story_prompt)
        ...
        st.write_stream(stream_wrapper(stream))
...
mood = st.text_input("Story Mood", placeholder="funny, sad, happy, etc")
language = st.text_input("Story Language", placeholder="English, Tamil, French, etc")
